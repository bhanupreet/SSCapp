...
public class AddCompActivity extends AppCompatActivity {
    ...
    private TextInputLayout mCompanyName;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAddBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String name = mCompanyName.getEditText().getText().toString();


                if(!name.equals("")) {
                    // DatabaseReference companyref = FirebaseDatabase.getInstance().getReference("Comapnies").push();
                    FirebaseDatabase database = FirebaseDatabase.getInstance();
                    DatabaseReference myRef = database.getReference().child("Companies").child(name);
